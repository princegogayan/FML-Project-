...
def search(request):
    ...
    df = pd.read_csv('D:/FML/MovieRecommender/data/movies.csv').merge(pd.read_csv('D:/FML/MovieRecommender/data/ratings.csv'), on='movieId')

    # Check for the movie name in the database
    movie_in_db = df['title'] == res
    ...
    if not np.any(movie_in_db):
        ...
        added_titles = []
        movie_name_words = [w.strip() for w in res.lower().split(' ')]
        ...
        # Ask user for input
        print()
        print('Movie with title "{}", can\'t be found in the "data/movies.csv" file.'.format(res))
    ...
    for user in movie_db.iloc[:5]['userId'].values:
        ...
        rated_movies = rated_movies[rated_movies['title'] != res] \
